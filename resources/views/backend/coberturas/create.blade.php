...
@section('header', isset($cobertura) ? 'Editar Cobertura' : 'Nueva Cobertura')
        ...
        <div class="bg-gradient-to-r from-sky-50 to-emerald-50 px-8 py-6 border-b border-slate-100">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h2 class="text-xl font-bold text-slate-800">
                    {{ isset($cobertura) ? 'Modificar Registro' : 'Registrar Cobertura' }}
                </h2>
            </div>
            <p class="text-sm text-slate-500 mt-2">Ingresa el nombre descriptivo de la cobertura para el sistema.</p>
        </div>

        <form action="{{ isset($cobertura) ? route('coberturas.update', $cobertura->id) : route('coberturas.store') }}" 
              method="POST" 
              class="p-8 space-y-6">
            @csrf
            @if(isset($cobertura)) @method('PUT') @endif

            <div class="space-y-2">
                <label for="nombre" class="text-sm font-bold text-slate-700 ml-1">Nombre de la Cobertura</label>
                <div class="relative group">
                    ...
                    <input type="text" name="nombre" id="nombre"
                           ...
                           placeholder="Ej: Obra Social" required>
                </div>
            </div>
            ...
            <div class="pt-4 flex items-center gap-3">
                <button type="submit" 
                    ...
                    {{ isset($cobertura) ? 'Guardar Cambios' : 'Registrar Cobertura' }}
                </button>

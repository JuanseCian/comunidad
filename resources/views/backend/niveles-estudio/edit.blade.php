...
@section('header', isset($nivel) ? 'Editar Nivel' : 'Nuevo Nivel')

@section('content')
<div class="max-w-xl mx-auto">
    <a href="{{ route('niveles-estudio.index') }}" class="inline-flex items-center text-sm text-slate-500 hover:text-emerald-600 mb-6 transition-colors">
        ...
        <div class="bg-gradient-to-r from-sky-50 to-emerald-50 px-8 py-6 border-b border-slate-100">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h2 class="text-xl font-bold text-slate-800">
                    {{ isset($nivel) ? 'Actualizar Formación' : 'Registrar Formación' }}
                </h2>
            </div>
            <p class="text-sm text-slate-500 mt-2">Define el nombre del nivel académico (ej: Secundario Completo).</p>
        </div>

        <form action="{{ isset($nivel) ? route('niveles-estudio.update', $nivel->id) : route('niveles-estudio.store') }}" 
              method="POST" 
              class="p-8 space-y-6">
            @csrf
            @if(isset($nivel)) @method('PUT') @endif

            <div class="space-y-2">
                <label for="nombre" class="text-sm font-bold text-slate-700 ml-1">Nombre del Nivel</label>
                <div class="relative group">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <i data-lucide="graduation-cap" class="w-5 h-5 text-slate-400 group-focus-within:text-emerald-500 transition-colors"></i>
                    </div>
                    <input type="text" name="nombre" id="nombre"
                           ...
                           value="{{ $nivel->nombre ?? '' }}" 
                           placeholder="Ej: Terciario en curso" required>
                </div>
            </div>
            ...
            <div class="pt-4 flex items-center gap-3">
                <button type="submit" 
                    ...
                    {{ isset($nivel) ? 'Guardar Cambios' : 'Registrar Nivel' }}
                </button>

...
@section('header', 'Niveles de Estudio')

@section('content')
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Niveles de Estudio</h1>
            <p class="text-sm text-slate-500">Gestiona las categorías de formación académica del sistema.</p>
        </div>
        <a href="{{ route('niveles-estudio.create') }}" 
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gradient-to-r from-sky-50/50 to-emerald-50/50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Descripción del Nivel</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-6 py-4 text-sm font-medium text-slate-400">#{{ $n->id }}</td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="p-2 bg-sky-50 text-sky-600 rounded-lg border border-sky-100">
                                    <i data-lucide="graduation-cap" class="w-4 h-4"></i>
                                </div>
                                <span class="text-sm font-semibold text-slate-700">{{ $n->nombre }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('niveles-estudio.edit', $n->id) }}" 
                                   class="p-2 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-all"
                                   title="Editar">
                                    <i data-lucide="edit-3" class="w-4 h-4"></i>
                                </a>

                                <form action="{{ route('niveles-estudio.destroy', $n->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" 
                                    onclick="confirmDelete(this.form, 'nivel de estudio', '{{ $n->nombre }}')"

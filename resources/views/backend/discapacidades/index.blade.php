...
@section('header', 'Gestión de Discapacidades')

@section('content')
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Discapacidades</h1>
            <p class="text-sm text-slate-500">Administra los tipos de discapacidad disponibles en el sistema.</p>
        </div>
        <a href="{{ route('discapacidades.create') }}" 
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gradient-to-r from-sky-50/50 to-emerald-50/50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-500">Tipo de Discapacidad</th>
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('discapacidades.edit', $item->id) }}" 
                                   class="p-2 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-all"
                                   title="Editar">
                                    <i data-lucide="edit-3" class="w-4 h-4"></i>
                                </a>

                                <form action="{{ route('discapacidades.destroy', $item->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                            onclick="confirmDelete(this.form, 'discapacidad', '{{ $item->nombre }}')"

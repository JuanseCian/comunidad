...
@section('header', isset($beneficio) ? 'Editar Beneficio' : 'Nuevo Beneficio')

@section('content')
<div class="max-w-xl mx-auto">
    <a href="{{ route('beneficios.index') }}" class="inline-flex items-center text-sm text-slate-500 hover:text-emerald-600 mb-6 transition-colors">
        ...
        <div class="bg-gradient-to-r from-sky-50 to-emerald-50 px-8 py-6 border-b border-slate-100">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h2 class="text-xl font-bold text-slate-800">
                    {{ isset($beneficio) ? 'Modificar Registro' : 'Registrar Beneficio' }}
                </h2>
            </div>
            <p class="text-sm text-slate-500 mt-2">Ingresa el nombre descriptivo del beneficio para el sistema.</p>
        </div>

        <form action="{{ isset($beneficio) ? route('beneficios.update', $beneficio->id) : route('beneficios.store') }}" 
              method="POST" 
              class="p-8 space-y-6">
            @csrf
            @if(isset($beneficio)) @method('PUT') @endif

            <div class="space-y-2">
                <label for="nombre" class="text-sm font-bold text-slate-700 ml-1">Nombre del Beneficio</label>
                <div class="relative group">
                    ...
                    <input type="text" name="nombre" id="nombre"
                           ...
                           placeholder="Ej: Asignación por Hijo" required>
                </div>
            </div>
            ...
            <div class="pt-4 flex items-center gap-3">
                <button type="submit" 
                    ...
                    {{ isset($beneficio) ? 'Guardar Cambios' : 'Registrar Beneficio' }}
                </button>

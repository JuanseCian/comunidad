...
@section('header', isset($categoria) ? 'Editar Categoría' : 'Nueva Categoría')

@section('content')
<div class="max-w-2xl mx-auto">
    <a href="{{ route('categorias.index') }}" class="inline-flex items-center text-sm text-slate-500 hover:text-emerald-600 mb-6 transition-colors">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 overflow-hidden">
        <div class="bg-gradient-to-r from-sky-50 to-emerald-50 px-8 py-6 border-b border-slate-100">
            <h2 class="text-xl font-bold text-slate-800">
                {{ isset($categoria) ? 'Modificar Categoría' : 'Registrar Categoría' }}
            </h2>
            <p class="text-sm text-slate-500 mt-1">Define una categoría ocupacional dentro del sistema.</p>
        </div>
        ...
        <form method="POST" 
            ...
            <div class="space-y-2">
                <label class="text-sm font-bold text-slate-700 ml-1">Nombre de la Categoría</label>
                <div class="relative group">
                    ...
                    <input type="text" name="nombre" value="{{ $categoria->nombre ?? '' }}" 
                           ...
                           placeholder="Ej: Empleado, Desocupado, Estudiante" required>
                </div>
            </div>

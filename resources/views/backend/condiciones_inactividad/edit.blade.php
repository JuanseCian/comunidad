...
@section('header', isset($condicion_inactividad) ? 'Editar Condición' : 'Nueva Condición')

@section('content')
<div class="max-w-xl mx-auto">
    <a href="{{ route('condiciones-inactividad.index') }}" class="inline-flex items-center text-sm text-slate-500 hover:text-emerald-600 mb-6 transition-colors">
        ...
        <div class="bg-gradient-to-r from-sky-50 to-emerald-50 px-8 py-6 border-b border-slate-100">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h2 class="text-xl font-bold text-slate-800">
                    {{ isset($condicion_inactividad) ? 'Modificar Registro' : 'Registrar Condición' }}
                </h2>
            </div>
            <p class="text-sm text-slate-500 mt-2">Ingresa la descripción de la condición de inactividad.</p>
        </div>

        <form action="{{ isset($condicion_inactividad) ? route('condiciones-inactividad.update', $condicion_inactividad->id) : route('condiciones-inactividad.store') }}" 
              method="POST" 
              class="p-8 space-y-6">
            @csrf
            @if(isset($condicion_inactividad)) @method('PUT') @endif

            <div class="space-y-2">
                <label for="nombre" class="text-sm font-bold text-slate-700 ml-1">Descripción</label>
                <div class="relative group">
                    ...
                    <input type="text" name="nombre" id="nombre"
                           ...
                           value="{{ $condicion_inactividad->nombre ?? '' }}" 
                           placeholder="Ej: Jubilado/a, Estudiante" required>
                </div>
            </div>
            ...
            <div class="pt-4 flex items-center gap-3">
                <button type="submit" 
                    ...
                    {{ isset($condicion_inactividad) ? 'Guardar Cambios' : 'Registrar Condición' }}
                </button>
                <a href="{{ route('condiciones-inactividad.index') }}" 

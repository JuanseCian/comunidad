...
@section('header', isset($enfermedad) ? 'Editar Enfermedad' : 'Nueva Enfermedad')

@section('content')
<div class="max-w-xl mx-auto">
    <a href="{{ route('enfermedades.index') }}" class="inline-flex items-center text-sm text-slate-500 hover:text-emerald-600 mb-6 transition-colors">
        ...
        <div class="bg-gradient-to-r from-sky-50 to-emerald-50 px-8 py-6 border-b border-slate-100">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h2 class="text-xl font-bold text-slate-800">
                    {{ isset($enfermedad) ? 'Modificar Registro' : 'Registrar Patología' }}
                </h2>
            </div>
            <p class="text-sm text-slate-500 mt-2">Ingresa la denominación de la enfermedad o condición médica.</p>
        </div>

        <form action="{{ isset($enfermedad) ? route('enfermedades.update', $enfermedad->id) : route('enfermedades.store') }}" 
              method="POST" 
              class="p-8 space-y-6">
            @csrf
            ...
            <div class="space-y-2">
                <label for="nombre" class="text-sm font-bold text-slate-700 ml-1">Nombre de la Enfermedad</label>
                <div class="relative group">
                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <i data-lucide="activity" class="w-5 h-5 text-slate-400 group-focus-within:text-emerald-500 transition-colors"></i>
                    </div>
                    <input type="text" name="nombre" id="nombre"
                           ...
                           placeholder="Ej: Diabetes Tipo 2" required>
                </div>
            </div>
            ...
            <div class="pt-4 flex items-center gap-3">
                <button type="submit" 
                    ...
                    {{ isset($enfermedad) ? 'Guardar Cambios' : 'Registrar Enfermedad' }}
                </button>

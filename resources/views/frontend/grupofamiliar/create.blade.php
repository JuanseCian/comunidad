...
@section('title', 'Agregar integrante — ' . $persona->apellido . ', ' . $persona->nombre)
...
<div style="background: linear-gradient(135deg, #e6f5fb 0%, #e8f9f5 100%); border-bottom: 1px solid #d0eee7; padding: 2rem 0 1.5rem;">
    <div class="container">
        ...
        <div style="display:inline-flex; align-items:center; gap:8px; background:white; border:1px solid #b3e0f5; color:#0879a8; border-radius:20px; padding:5px 14px; font-size:12.5px; font-weight:700;">
            ...
            <span style="opacity:.5; font-weight:400;">· DNI {{ $persona->dni }}</span>
        </div>
    </div>
</div>
...
<div style="max-width:780px; margin:0 auto;">
                ...
                <div class="col-md-6">
                    ...
                    <select name="documento_id" class="{{ $errors->has('documento_id') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($catalogos['tipos_documento'] as $td)
                            <option value="{{ $td->id }}" {{ old('documento_id') == $td->id ? 'selected' : '' }}>{{ $td->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="sexo_id" class="{{ $errors->has('sexo_id') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($catalogos['sexos'] as $s)
                            <option value="{{ $s->id }}" {{ old('sexo_id') == $s->id ? 'selected' : '' }}>{{ $s->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="estado_civil_id" class="{{ $errors->has('estado_civil_id') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($catalogos['estados_civiles'] as $ec)
                            <option value="{{ $ec->id }}" {{ old('estado_civil_id') == $ec->id ? 'selected' : '' }}>{{ $ec->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div style="display:flex; align-items:center; justify-content:space-between; padding:13px 0; border-bottom:1px solid #f0ede8;">
                    ...
                    <label style="position:relative; display:inline-block; width:42px; height:24px; flex-shrink:0; cursor:pointer; margin:0;">
                        <input type="checkbox" name="discapacidad_permanente" value="1" id="chk-discapacidad"
                               ...
                               style="opacity:0; width:0; height:0; position:absolute;">
                        <span class="toggle-slider" style="position:absolute; inset:0; background:{{ old('discapacidad_permanente') ? '#0d92c2' : '#c8c4bb' }}; border-radius:24px; transition:background .2s;">
                            <span style="position:absolute; top:3px; left:3px; width:18px; height:18px; background:white; border-radius:50%; transition:transform .2s; box-shadow:0 1px 3px rgba(0,0,0,.15); display:block; transform:{{ old('discapacidad_permanente') ? 'translateX(18px)' : 'translateX(0)' }};"></span>
                        </span>
                    </label>
                </div>
                <div class="cond-block {{ old('discapacidad_permanente') ? 'visible' : '' }}" id="blk-discapacidad"
                     style="display:{{ old('discapacidad_permanente') ? 'block' : 'none' }}; margin:0; padding:16px; background:#f8fafe; border-radius:10px; border:1px solid #e0ddd6; margin-top:8px; margin-bottom:8px;">
                    <div class="row g-3">
                        <div class="col-md-6">
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; padding:12px 0 0; margin-top:12px; border-top:1px solid #e0ddd6;">
                        ...
                        <label style="position:relative; display:inline-block; width:42px; height:24px; flex-shrink:0; cursor:pointer; margin:0;">
                            <input type="checkbox" name="discapacidad_tratamiento" value="1"
                                   ...
                                   style="opacity:0; width:0; height:0; position:absolute;">
                            <span class="toggle-slider" style="position:absolute; inset:0; background:{{ old('discapacidad_tratamiento') ? '#0d92c2' : '#c8c4bb' }}; border-radius:24px; transition:background .2s;">
                                <span style="position:absolute; top:3px; left:3px; width:18px; height:18px; background:white; border-radius:50%; transition:transform .2s; box-shadow:0 1px 3px rgba(0,0,0,.15); display:block; transform:{{ old('discapacidad_tratamiento') ? 'translateX(18px)' : 'translateX(0)' }};"></span>
                            </span>
                        </label>
                    </div>
                </div>
                ...
                <div style="display:flex; align-items:center; justify-content:space-between; padding:13px 0; border-bottom:1px solid #f0ede8;">
                    ...
                    <label style="position:relative; display:inline-block; width:42px; height:24px; flex-shrink:0; cursor:pointer; margin:0;">
                        <input type="checkbox" name="_tiene_enfermedad" value="1" id="chk-enfermedad"
                               ...
                               style="opacity:0; width:0; height:0; position:absolute;">
                        <span class="toggle-slider" style="position:absolute; inset:0; background:{{ old('enfermedad_id') ? '#0d92c2' : '#c8c4bb' }}; border-radius:24px; transition:background .2s;">
                            <span style="position:absolute; top:3px; left:3px; width:18px; height:18px; background:white; border-radius:50%; transition:transform .2s; box-shadow:0 1px 3px rgba(0,0,0,.15); display:block; transform:{{ old('enfermedad_id') ? 'translateX(18px)' : 'translateX(0)' }};"></span>
                        </span>
                    </label>
                </div>
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; padding:12px 0 0; margin-top:12px; border-top:1px solid #e0ddd6;">
                        ...
                        <label style="position:relative; display:inline-block; width:42px; height:24px; flex-shrink:0; cursor:pointer; margin:0;">
                            <input type="checkbox" name="enfermedad_tratamiento" value="1"
                                   ...
                                   style="opacity:0; width:0; height:0; position:absolute;">
                            <span class="toggle-slider" style="position:absolute; inset:0; background:{{ old('enfermedad_tratamiento') ? '#0d92c2' : '#c8c4bb' }}; border-radius:24px; transition:background .2s;">
                                <span style="position:absolute; top:3px; left:3px; width:18px; height:18px; background:white; border-radius:50%; transition:transform .2s; box-shadow:0 1px 3px rgba(0,0,0,.15); display:block; transform:{{ old('enfermedad_tratamiento') ? 'translateX(18px)' : 'translateX(0)' }};"></span>
                            </span>
                        </label>
                    </div>
                </div>
                ...
                <div style="display:flex; align-items:center; justify-content:space-between; padding:13px 0; border-bottom:1px solid #f0ede8;">
                    ...
                    <label style="position:relative; display:inline-block; width:42px; height:24px; flex-shrink:0; cursor:pointer; margin:0;">
                        <input type="checkbox" name="embarazo" value="1" id="chk-embarazo"
                               ...
                               style="opacity:0; width:0; height:0; position:absolute;">
                        <span class="toggle-slider" style="position:absolute; inset:0; background:{{ old('embarazo') ? '#0d92c2' : '#c8c4bb' }}; border-radius:24px; transition:background .2s;">
                            <span style="position:absolute; top:3px; left:3px; width:18px; height:18px; background:white; border-radius:50%; transition:transform .2s; box-shadow:0 1px 3px rgba(0,0,0,.15); display:block; transform:{{ old('embarazo') ? 'translateX(18px)' : 'translateX(0)' }};"></span>
                        </span>
                    </label>
                </div>
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between;">
                        ...
                        <label style="position:relative; display:inline-block; width:42px; height:24px; flex-shrink:0; cursor:pointer; margin:0;">
                            <input type="checkbox" name="control_embarazo" value="1"
                                   ...
                                   style="opacity:0; width:0; height:0; position:absolute;">
                            <span class="toggle-slider" style="position:absolute; inset:0; background:{{ old('control_embarazo') ? '#0d92c2' : '#c8c4bb' }}; border-radius:24px; transition:background .2s;">
                                <span style="position:absolute; top:3px; left:3px; width:18px; height:18px; background:white; border-radius:50%; transition:transform .2s; box-shadow:0 1px 3px rgba(0,0,0,.15); display:block; transform:{{ old('control_embarazo') ? 'translateX(18px)' : 'translateX(0)' }};"></span>
                            </span>
                        </label>
                    </div>
                </div>
                ...
                <div style="display:flex; align-items:center; justify-content:space-between; padding:13px 0;">
                    ...
                    <label style="position:relative; display:inline-block; width:42px; height:24px; flex-shrink:0; cursor:pointer; margin:0;">
                        <input type="checkbox" name="esquema_vacunacion" value="1"
                               ...
                               style="opacity:0; width:0; height:0; position:absolute;">
                        <span class="toggle-slider" style="position:absolute; inset:0; background:{{ old('esquema_vacunacion') ? '#0d92c2' : '#c8c4bb' }}; border-radius:24px; transition:background .2s;">
                            <span style="position:absolute; top:3px; left:3px; width:18px; height:18px; background:white; border-radius:50%; transition:transform .2s; box-shadow:0 1px 3px rgba(0,0,0,.15); display:block; transform:{{ old('esquema_vacunacion') ? 'translateX(18px)' : 'translateX(0)' }};"></span>
                        </span>
                    </label>
                </div>
                ...
                <div class="col-md-6" id="blk-inactividad" style="display:none">
                    ...
                    <select name="condicion_inactividad_id"
                        ...
                        @foreach($catalogos['condiciones_inactividad'] as $ci)
                            <option value="{{ $ci->id }}" {{ old('condicion_inactividad_id') == $ci->id ? 'selected' : '' }}>{{ $ci->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select name="categoria_ocupacional_id"
                        ...
                        @foreach($catalogos['categorias_ocupacional'] as $co)
                            <option value="{{ $co->id }}" {{ old('categoria_ocupacional_id') == $co->id ? 'selected' : '' }}>{{ $co->nombre }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; padding-top:1rem; border-top:1px solid #e0ddd6; margin-top:.5rem;">
        ...
        <button type="submit"
            ...
            <i class="bi bi-person-check-fill"></i> Guardar integrante
        </button>
    </div>

...
<style>
    ...
    .sp-alert {
        ...
        font-size: 13.5px; font-weight: 600; margin-bottom: 1.5rem;
    }
    ...
    .sp-card-header {
        padding: 13px 20px 12px; border-bottom: 1px solid var(--border-sm);
...
@if($alerta)
    <div id="modalSugerenciaPrograma"
        ...
        <div class="sp-anim" style="background:white; border-radius:24px; width:90%; max-width:450px; box-shadow:0 25px 50px -12px rgba(0,0,0,0.25); overflow:hidden;">
            ...
            <div style="background:linear-gradient(135deg,#fff7ed,#ffedd5); padding:30px 24px; text-align:center; border-bottom:1px solid #fed7aa;">
                <div style="width:60px; height:60px; background:#fb923c; color:white; border-radius:18px; display:flex; align-items:center; justify-content:center; margin:0 auto 15px; font-size:28px; box-shadow:0 10px 15px -3px rgba(251,146,60,.4);">
            ...
            <div style="padding:24px; text-align:center;">
                <p style="font-size:14.5px; color:#7c2d12; line-height:1.6; margin-bottom:20px;">{{ $alerta['mensaje'] }}</p>
...
<div class="sp-page">
    ...
    <section style="background:linear-gradient(135deg, var(--blue-lt) 0%, var(--teal-lt) 100%); border-bottom:1px solid #d0eee7; padding:2rem 0 1.75rem;">
        <div class="container">
            ...
            <nav class="sp-breadcrumb sp-anim sp-anim-1">
                ...
                <span>/</span>
                <span style="color:var(--slate); font-weight:600;">{{ $persona->apellido }}, {{ $persona->nombre }}</span>
            </nav>
                    ...
                    <div>
                        ...
                        <div style="display:flex; flex-wrap:wrap; gap:6px;">
                            ...
                            @if($persona->fecha_nacimiento)
                                <span class="sp-pill sp-pill-gray">{{ $edad }} años</span>
                            @endif
                            @if($persona->sedeOrigen)
                                <span class="sp-pill sp-pill-blue">
                                    <i class="bi bi-geo-alt-fill" style="font-size:10px; margin-right:3px;"></i>
                                    {{ $persona->sedeOrigen->nombre }}
                                </span>
                            @endif
                            @if($persona->trabaja)
                                <span class="sp-pill sp-pill-teal">
                                    <i class="bi bi-briefcase-fill" style="font-size:10px; margin-right:3px;"></i>
                                    Trabaja
                                </span>
                            @endif
                        </div>
    ...
    <div class="container py-4">
        ...
        {{-- ── Fila 1: Datos personales · Domicilio ── --}}
                ...
                <div class="sp-card">
                    <div class="sp-card-header">
                        ...
                        @if(in_array(auth()->user()->rol_id, [2,3,5]))
                            <button type="button" onclick="toggleEdit('datos')" class="sp-card-action">Editar</button>
                        @endif
                    </div>
                    <div class="sp-card-body">
                        <form action="{{ route('personas.updateDatos', $persona->id) }}" method="POST">
                            ...
                            <div class="sp-data-row">
                                ...
                                <input type="email" name="correo" value="{{ $persona->correo }}" class="edit-datos" style="display:none;">
                            </div>

                            <div class="sp-data-row">
                                <span class="sp-label"><i class="bi bi-phone"></i> Teléfono</span>
                                <span class="sp-val view-datos">{{ $persona->telefono ?? '—' }}</span>
                                <input type="text" name="telefono" value="{{ $persona->telefono }}" class="edit-datos" style="display:none;">
                            </div>
                            ...
                            <div class="sp-data-row">
                                ...
                                <input type="text" name="cuil" value="{{ $persona->cuil }}" class="edit-datos" style="display:none;">
                            </div>
                            ...
                            <div class="sp-data-row">
                                ...
                                <input type="text" name="grupo_sanguineo" value="{{ $persona->grupo_sanguineo }}" class="edit-datos" style="display:none;">
                            </div>
                            ...
                            <div class="sp-data-row">
                                ...
                                <select name="nivel_estudio_id" class="edit-datos" style="display:none;">
                                    ...
                                    @foreach($niveles as $n)
                                        <option value="{{ $n->id }}" @selected($persona->nivel_estudio_id == $n->id)>{{ $n->nombre }}</option>
                                    @endforeach
                                </select>
                            </div>
            ...
            <div class="col-md-6 sp-anim sp-anim-3">
                <div class="sp-card">
                    <div class="sp-card-header">
                        ...
                        @if(in_array(auth()->user()->rol_id, [2,3,5]))
                            <button type="button" onclick="toggleEdit('domicilio')" class="sp-card-action">Editar</button>
                        @endif
                    </div>
                    <div class="sp-card-body">
                        <form action="{{ route('personas.updateDomicilio', $persona->id) }}" method="POST">
                            ...
                            <div class="sp-data-row">
                                ...
                                <select name="provincia_id" class="edit-domicilio" style="display:none;">
                                    ...
                                    @foreach($provincias as $p)
                                        <option value="{{ $p->id }}" @selected($persona->provincia_id == $p->id)>{{ $p->nombre }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="sp-data-row">
                                ...
                                <select name="localidad_id" class="edit-domicilio" style="display:none;">
                                    ...
                                    @foreach($localidades as $l)
                                        <option value="{{ $l->id }}" @selected($persona->localidad_id == $l->id)>{{ $l->nombre }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="sp-data-row">
                                ...
                                <select name="barrio_id" class="edit-domicilio" style="display:none;">
                                    ...
                                    @foreach($barrios as $b)
                                        <option value="{{ $b->id }}" @selected(optional($persona->domicilio)->barrio_id == $b->id)>{{ $b->nombre }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="sp-data-row">
                                ...
                                <input type="text" name="calle"   value="{{ $persona->domicilio->calle   ?? '' }}" class="edit-domicilio" style="display:none;">
                                <input type="text" name="numero"  value="{{ $persona->domicilio->numero  ?? '' }}" class="edit-domicilio" style="display:none; width:80px;">
                            </div>
                            ...
                            <div class="sp-data-row">
                                ...
                                <input type="text" name="piso" value="{{ $persona->domicilio->piso ?? '' }}" class="edit-domicilio" style="display:none; width:60px;">
                                <input type="text" name="dpto" value="{{ $persona->domicilio->dpto ?? '' }}" class="edit-domicilio" style="display:none; width:60px;">
                            </div>
                ...
                <div class="sp-card">
                    <div class="sp-card-header">
                        ...
                        <button onclick="document.getElementById('modalPrograma').style.display='flex'"
                                class="sp-card-action" style="cursor:pointer; font-family:var(--sans);">
                            + Asignar
                        </button>
                    </div>
                    <div class="sp-card-body">
                        @forelse($persona->personaPrograma as $pp)
                            ...
                            <div style="border:1px solid var(--border); border-radius:12px; margin-bottom:10px; overflow:hidden;">
                                ...
                                <div onclick="togglePrograma({{ $pp->id }})"
                                    ...
                                    <div style="display:flex; flex-direction:column; gap:2px;">
                                        <span style="font-weight:700;">{{ $pp->programa->nombre }}</span>
                                        <div style="display:flex; align-items:center; gap:6px;">
                                            <i class="bi bi-geo-alt-fill" style="font-size:11px; color:var(--teal-dk);"></i>
                                            <span style="font-size:11px; color:var(--muted); font-weight:600;">{{ $pp->sede->nombre ?? 'Sede única' }}</span>
                                        </div>
                                    </div>
                        ...
                        @empty
                            <div class="sp-empty">Sin programas asignados</div>
                        @endforelse
                    </div>
                </div>
            </div>
        ...
        {{-- ── Fila 3: Beneficios + Mercadería Familiar ── --}}
        <div class="row g-3 mb-3">
            <div class="col-12 sp-anim sp-anim-4">
                ...
                <div class="sp-card">
                    ...
                    <div class="sp-card-body">
                        ...
                        @php
                            $tieneBeneficios = $persona->personaBeneficio && $persona->personaBeneficio->count();
                        ...
                        @if($tieneBeneficios || $tieneMercaderia)
                            ...
                            <div class="row g-3">
                                ...
                                @if($tieneMercaderia)
                                    ...
                                    @foreach($persona->familia->mercaderiasActivas as $mercaderia)
                                                ...
                                                ">
                                                    ...
                                                    @if($mercaderia->usuario)
                                                        ...
                                                        <div>
                                                            ...
                                                            <strong>
                                                                {{ $mercaderia->usuario->nombre }} {{ $mercaderia->usuario->apellido }}
                                                            </strong>
                                                        </div>
                        ...
                        @else
                            ...
                            <div class="sp-empty" style="padding:40px 20px;">
                                ...
                                <div style="
                                    ...
                                    font-size:13px;
                                ">
                                    Esta persona todavía no posee beneficios ni mercadería activa.
                                </div>
                                ...
                                @foreach($integrantesFamilia as $integrante)
                                    ...
                                    <div style="display:flex; align-items:center; gap:10px; padding:9px 13px; background:{{ $esCurrent ? 'var(--blue-lt)' : 'var(--bg)' }}; border-radius:10px; border:1px solid {{ $esCurrent ? 'var(--blue-brd)' : 'var(--border)' }};">
                                        <div style="width:32px; height:32px; border-radius:50%; background:{{ $esCurrent ? 'linear-gradient(135deg,var(--blue),var(--teal))' : 'var(--border)' }}; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                                            <span style="font-size:12px; font-weight:800; color:{{ $esCurrent ? 'white' : 'var(--slate)' }};">
                                        ...
                                        @if(!$esCurrent)
                                            <a href="{{ route('personas.show', $integrante->id) }}" class="sp-card-action" style="font-size:11px;">Ver →</a>
                                            @if(in_array(auth()->user()->rol_id, [2,3,5]))
                                                <form method="POST" action="{{ route('personas.familia.desvincular', $integrante->id) }}"
                                                    ...
                                                    <button type="submit" title="Desvincular"
                                                            ...
                                                            onmouseover="this.style.background='#fecaca'" onmouseout="this.style.background='#fff5f5'">
                                                ...
                                                <tbody>
                                                    @foreach($convivientes as $m)
                                                        <tr>
                                                            ...
                                                            <td style="color:var(--slate); font-size:12.5px;">{{ $m->direccion ?? '—' }}</td>
                                                            <td>
                                            ...
                                            <table class="sp-table" style="margin:0;">
                                                <thead>
                                                    <tr style="background:#fffbeb;">
                                                        ...
                                                        <th style="color:#b45309;">Dirección</th>
                            ...
                            <div style="border-top:1px solid var(--border-sm); padding-top:16px; margin-top:16px;">
                                <a href="{{ route('familias.show', $persona->familia->id) }}" class="sp-btn-primary"
                                   style="text-decoration:none; display:inline-flex; align-items:center; gap:8px;">
                                    <i class="bi bi-people-fill"></i> Ver grupo familiar completo
                                </a>
                            </div>
                        @endif
                                    ...
                                    <div style="display:flex; gap:8px; align-items:flex-start;">
                                        <div style="flex:1;">
                                            <input type="text" name="codigo"
                                                   placeholder="Ej: KAR-482-XTQ" maxlength="11" autocomplete="off"
        ...
        <div class="row g-3 mb-3">
            <div class="col-12 sp-anim">
                <div class="sp-card">
                    ...
                    <div class="sp-card-header" style="flex-wrap:wrap; gap:10px;">
                        <div style="display:flex; align-items:center; gap:10px;">
                            <div style="width:34px; height:34px; border-radius:50%; background:linear-gradient(135deg,var(--blue),#1aaad8); display:flex; align-items:center; justify-content:center; color:white; font-size:14px; flex-shrink:0;">
                        ...
                        @if(Auth::user()->puedeEditar())
                            <button type="button"
                                    ...
                                    class="sp-btn-primary" style="height:36px; font-size:12.5px;">
                    ...
                    @else
                        <div class="sp-empty" style="padding:28px;">
                            <div class="sp-empty-icon" style="background:#f0fdf8; color:var(--blue);"><i class="bi bi-briefcase"></i></div>
            ...
            <div class="col-12 sp-anim sp-anim-6">
                <div class="sp-card">
                    ...
                    <div class="sp-card-header">
                        <div class="sp-card-header-left">
                            <div class="sp-dot" style="background:var(--teal);"></div>
                            <span class="sp-card-title">Intervenciones</span>
                            @if($persona->atenciones)
                                <span class="sp-count">{{ $persona->atenciones->count() }}</span>
                            @endif
                        </div>
                        @if(Auth::user()->puedeEditar())
                            <a href="{{ route('atenciones.create', $persona) }}" class="sp-btn-primary">
                    ...
                    <div class="sp-card-body">
                                        ...
                                        <div style="display:flex; justify-content:space-between; gap:15px; flex-wrap:wrap;">
                                            <div style="flex:1; min-width:260px;">
                                                <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
                                                    ...
                                                    <span style="font-size:11.5px; color:var(--muted);">
                                                        ...
                                                        {{ $a->fecha_atencion ? \Carbon\Carbon::parse($a->fecha_atencion)->format('d/m/Y') : 'Sin fecha' }}
                                                    </span>
                                                </div>
                                                ...
                                                <div style="font-size:11.5px; color:var(--muted); margin-top:10px;">
                                                    <i class="bi bi-person"></i> {{ $a->users->nombre ?? 'Sistema' }}
                                                </div>
                                            </div>
                                                    ...
                                                    @foreach($a->adjuntos as $adj)
                                                        <div style="display:flex; align-items:center; justify-content:space-between; padding:9px 13px; border-bottom:1px solid var(--border-sm); font-size:12.5px;">
                                                            <div style="display:flex; align-items:center; gap:8px;">
                                                                <i class="bi {{ str_contains($adj->tipo_mime ?? '', 'pdf') ? 'bi-file-earmark-pdf text-danger' : 'bi-file-earmark-image' }}" style="font-size:15px;"></i>
                                            ...
                                            <div style="display:flex; align-items:start; gap:8px;">
                                                @if(Auth::user()->puedeVerAtenciones())
                                                    <a href="{{ route('atenciones.show', $a) }}" class="sp-btn-ghost">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                                                @endif

                                                @if($a->adjuntos && $a->adjuntos->count())
                                                    <button type="button" class="sp-btn-ghost"
                                                            title="{{ $a->adjuntos->count() }} archivo(s) adjunto(s)"
                                                            onclick="toggleAdjuntos({{ $a->id }})">
                                                        <i class="bi bi-paperclip"></i>
                                                        <span style="font-size:11px; font-weight:700;">{{ $a->adjuntos->count() }}</span>
                                                    </button>
                                                @endif

                                                @if(Auth::user()->puedeEditar())
                                                    <a href="{{ route('atenciones.edit', $a) }}" class="sp-btn-ghost">
                                                        <i class="bi bi-pencil-square"></i>
                                                    </a>
                                                    <form method="POST" action="{{ route('atenciones.destroy', $a) }}">
                                                        @csrf @method('DELETE')
                                                        <button type="submit" class="sp-btn-ghost sp-btn-delete"
                        ...
                        @else
                            <div class="sp-empty">
                                ...
                                <div>Sin intervenciones registradas</div>
                                @if(Auth::user()->puedeEditar())
                                    <a href="{{ route('atenciones.create', $persona) }}" class="sp-btn-primary" style="margin-top:15px;">
        ...
        <div style="padding:24px;">
            <form action="{{ route('persona-programa.store') }}" method="POST">
                ...
                <div style="margin-bottom:20px;">
                    <label style="display:block; font-size:12px; font-weight:800; color:var(--slate); margin-bottom:8px; text-transform:uppercase; letter-spacing:.06em;">Programa disponible</label>
                    <select name="programa_id" id="programa_select" required
                ...
                {{-- Sede --}}
                <div id="wrapper_sede" style="margin-bottom:20px;">
                    <label style="display:block; font-size:12px; font-weight:800; color:var(--slate); margin-bottom:8px; text-transform:uppercase; letter-spacing:.06em;">Sede</label>
                    <select name="sede_id" id="select_sede"
                ...
                {{-- Rol --}}
                <div style="margin-bottom:20px;">
                    <label style="display:block; font-size:12px; font-weight:800; color:var(--slate); margin-bottom:8px; text-transform:uppercase;">Rol en el programa</label>
                ...
                <div style="margin-bottom:12px;">
                    <label style="display:block; font-size:12px; font-weight:800; color:var(--slate); margin-bottom:6px;">Fecha inicio</label>
                ...
                <div style="margin-bottom:20px;">
                    <label style="display:block; font-size:12px; font-weight:800; color:var(--slate); margin-bottom:6px;">Fecha fin</label>
                    <input type="date" name="fecha_fin"
                           style="width:100%; padding:10px; border-radius:10px; border:1px solid var(--border);">
                </div>
...
{{-- Modal: Editar fechas de programa (legacy) --}}
    ...
    <div style="background:white; padding:20px; border-radius:12px; width:300px;">
        ...
        <form id="formEditarPrograma" method="POST">
            ...
            <div style="display:flex; gap:8px; margin-top:16px;">
                ...
                <button type="button" onclick="cerrarModalEditar()" class="sp-btn-ghost">Cancelar</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modalNuevoTrabajo"
    ...
    <div style="background:white; border-radius:16px; width:100%; max-width:580px; max-height:90vh; overflow-y:auto; box-shadow:0 20px 60px rgba(0,0,0,.25);">
        ...
        <form method="POST" action="{{ route('personas.trabajo.store', $persona) }}" style="padding:20px 22px;">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    <label style="font-size:11px; font-weight:700; color:var(--slate); text-transform:uppercase; letter-spacing:.08em; display:block; margin-bottom:5px;">Situación ocupacional</label>
                    <select name="situacion_ocupacional_id" style="width:100%; height:40px; padding:0 10px; border:1px solid var(--border); border-radius:10px; font-size:14px; background:white; color:var(--ink);">
                        <option value="">— Seleccionar —</option>
                ...
                <div class="col-md-6">
                    <label style="font-size:11px; font-weight:700; color:var(--slate); text-transform:uppercase; letter-spacing:.08em; display:block; margin-bottom:5px;">Categoría ocupacional</label>
                    <select name="categoria_ocupacional_id" style="width:100%; height:40px; padding:0 10px; border:1px solid var(--border); border-radius:10px; font-size:14px; background:white; color:var(--ink);">
                        <option value="">— Seleccionar —</option>
                ...
                <div class="col-md-6">
                    <label style="font-size:11px; font-weight:700; color:var(--slate); text-transform:uppercase; letter-spacing:.08em; display:block; margin-bottom:5px;">Rubro / Tipo <span style="color:#e74c3c;">*</span></label>
                    <input type="text" name="descripcion" value="{{ old('descripcion', $trabajoActual?->descripcion) }}"
                           placeholder="Ej: Albañilería, Docencia..."
                           style="width:100%; height:40px; padding:0 12px; border:1px solid var(--border); border-radius:10px; font-size:14px; font-family:inherit; color:var(--ink);" required>
                </div>
                <div class="col-md-6">
                    <label style="font-size:11px; font-weight:700; color:var(--slate); text-transform:uppercase; letter-spacing:.08em; display:block; margin-bottom:5px;">Empleador</label>
                    <input type="text" name="empleador" value="{{ old('empleador', $trabajoActual?->empleador) }}"
                           placeholder="Nombre del empleador"
                           style="width:100%; height:40px; padding:0 12px; border:1px solid var(--border); border-radius:10px; font-size:14px; font-family:inherit; color:var(--ink);">
                </div>
                <div class="col-md-6">
                    <label style="font-size:11px; font-weight:700; color:var(--slate); text-transform:uppercase; letter-spacing:.08em; display:block; margin-bottom:5px;">Cargo</label>
                ...
                <div class="col-md-6">
                    <label style="font-size:11px; font-weight:700; color:var(--slate); text-transform:uppercase; letter-spacing:.08em; display:block; margin-bottom:5px;">Fecha de inicio</label>
                    <input type="date" name="fecha_inicio" value="{{ old('fecha_inicio', now()->format('Y-m-d')) }}"
                           style="width:100%; height:40px; padding:0 12px; border:1px solid var(--border); border-radius:10px; font-size:14px; font-family:inherit; color:var(--ink);">
                </div>
                <div class="col-12">
                    <label style="font-size:11px; font-weight:700; color:var(--slate); text-transform:uppercase; letter-spacing:.08em; display:block; margin-bottom:5px;">Observaciones</label>
                    <textarea name="observaciones" rows="2" placeholder="Información adicional..."
                              style="width:100%; padding:8px 12px; border:1px solid var(--border); border-radius:10px; font-size:14px; font-family:inherit; color:var(--ink); resize:vertical;">{{ old('observaciones') }}</textarea>
                </div>
            </div>
            <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:20px; padding-top:16px; border-top:1px solid var(--border);">
                <button type="button" onclick="document.getElementById('modalNuevoTrabajo').style.display='none'"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (programaSelect && rolSelect) {
        programaSelect.addEventListener('change', function () {
            const texto = this.options[this.selectedIndex].text;
            [...rolSelect.options].forEach(o => {
                o.style.display = (o.value === 'tutor' && texto !== 'Envion') ? 'none' : 'block';
            });
            if (texto !== 'Envion') rolSelect.value = 'destinatario';
        });
    }
    ...
    if (checkAdaptacion) {
        checkAdaptacion.addEventListener('change', function () {
            wrapperFecha.style.display = this.checked ? 'block' : 'none';
            inputFecha.required = this.checked;
